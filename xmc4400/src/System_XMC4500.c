...
static int SystemClockSetup(void)
{
    ...
    if (AllowPLLInitByStartup()) {
        ...
        if ((SCU_PLL->
             PLLCON0 & (SCU_PLL_PLLCON0_VCOPWD_Msk |
                        SCU_PLL_PLLCON0_PLLPWD_Msk)) != 0) {
            /* enable PLL first */
            SCU_PLL->PLLCON0 &=
                ~(SCU_PLL_PLLCON0_VCOPWD_Msk | SCU_PLL_PLLCON0_PLLPWD_Msk);
        }
...
/* Enable OSC_HP if not already on*/
           ...
           /********************************/
                ...
                do {
                    ;           /* wait for ~150ms  */
                            ...
                            PLLSTAT) & (SCU_PLL_PLLSTAT_PLLHV_Msk |
        ...
        /********************************************************************************************************************/
            ...
            if (SCU_STANDBY_CLOCK == HIB_CLOCK_FOSI) {
                ...
                if ((SCU_HIBERNATE->
                     OSCULCTRL & SCU_HIBERNATE_OSCULCTRL_MODE_Msk) != 0) {
                    while (SCU_GENERAL->
                           MIRRSTS & SCU_GENERAL_MIRRSTS_OSCULCTRL_Msk) ;
                    ...
                    while (SCU_GENERAL->
                           MIRRSTS & SCU_GENERAL_MIRRSTS_HDCR_Msk) ;
                    ...
                    do {
                        ...
                        while (SCU_GENERAL->
                               MIRRSTS & SCU_GENERAL_MIRRSTS_HDCLR_Msk) ;
                        for (temp=0; temp <= 0xFFFF; temp++) ;
                    }
                    while ((SCU_HIBERNATE->
                            HDSTAT & SCU_HIBERNATE_HDSTAT_ULPWDG_Msk) ==
        ...
        if (!(SCU_PLL->PLLSTAT & SCU_PLL_PLLSTAT_VCOLOCK_Msk)) {
            ...
            if (SCU_PLL_CLOCK_INPUT == SCU_CLOCK_CRYSTAL)
                VCO =
                    (CLOCK_CRYSTAL_FREQUENCY / (SCU_PLL_PDIV + 1)) *
                    (SCU_PLL_NDIV + 1);
            if ((SCU_PLL_CLOCK_INPUT == SCU_CLOCK_BACK_UP_AUTOMATIC)
            ...
            SCU_PLL->PLLCON1 =
                ((SCU_PLL_K1DIV) | (SCU_PLL_NDIV << 8) | (stepping_K2DIV << 16)
                 | (SCU_PLL_PDIV << 24));
	   ...
	   *********************************************************/
            if (CLOCK_FSYS > 60000000) {
                ...
                if ((SCU_PLL_CLOCK_INPUT == SCU_CLOCK_BACK_UP_AUTOMATIC)
                    || (SCU_PLL_CLOCK_INPUT == SCU_CLOCK_BACK_UP_FACTORY))
                    VCO =
                        (CLOCK_BACK_UP / (SCU_PLL_PDIV + 1)) * (SCU_PLL_NDIV +
                                                                1);
                ...
                /* Setup devider settings for main PLL */
                SCU_PLL->PLLCON1 =
                    ((SCU_PLL_K1DIV) | (SCU_PLL_NDIV << 8) |
                     (stepping_K2DIV << 16) | (SCU_PLL_PDIV << 24));
            } else {
                /* Setup devider settings for main PLL */
                SCU_PLL->PLLCON1 =
                    ((SCU_PLL_K1DIV) | (SCU_PLL_NDIV << 8) |
   ...
   /*********************************************************
	   ...
	   *********************************************************/
           ...
           /********************************/
            ...
            /* Setup devider settings for main PLL */
            SCU_PLL->PLLCON1 =
                ((SCU_PLL_K1DIV) | (SCU_PLL_NDIV << 8) | (SCU_PLL_K2DIV << 16) |
                 (SCU_PLL_PDIV << 24));
...
static int USBClockSetup(void)
{
    ...
    if (AllowPLLInitByStartup()) {
        ...
        /* check if PLL is switched on */
        if ((SCU_PLL->
             USBPLLCON & (SCU_PLL_USBPLLCON_VCOPWD_Msk |
                          SCU_PLL_USBPLLCON_PLLPWD_Msk)) != 0) {
            /* enable PLL first */
            SCU_PLL->USBPLLCON &=
                ~(SCU_PLL_USBPLLCON_VCOPWD_Msk | SCU_PLL_USBPLLCON_PLLPWD_Msk);
        }
